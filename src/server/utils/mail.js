...
export async function sendInvoiceCreatedEmail(toEmail, username, invoice, plan) {
  const config = loadConfig();
  const panelName = config.panel?.name || 'Sodium Panel';
  const panelUrl = config.panel?.url || '';
  ...
  const content = `
    ...
    <div style="background-color: #0a0a0a; border: 1px solid #27272a; border-radius: 8px; padding: 16px; margin-bottom: 20px;">
      <table width="100%" style="color: #a1a1aa; font-size: 14px;">
        <tr>
          <td style="padding: 4px 0;">Plan:</td>
          <td style="text-align: right; color: #fafafa;">${plan?.name || 'Subscription'}</td>
        </tr>
        <tr>
          <td style="padding: 4px 0;">Amount:</td>
          <td style="text-align: right; color: #e07a3a; font-weight: 600;">${symbol}${invoice.total.toFixed(2)}</td>
        </tr>
        <tr>
          <td style="padding: 4px 0;">Due Date:</td>
          <td style="text-align: right; color: #fafafa;">${new Date(invoice.dueDate).toLocaleDateString()}</td>
        </tr>
      </table>
    </div>
    ${panelUrl ? emailButton('View Invoice', `${panelUrl}/billing`) : ''}
    ${emailNote("Please complete payment before the due date to avoid service interruption.")}
  `;
  
  return sendMail({
    to: toEmail,
    subject: `${panelName} - New Invoice Created`,
    html: emailTemplate(panelName, 'Invoice Created', content, `${panelName} • Billing notification`),
    text: `${panelName} - Invoice Created\n\nHi ${username},\n\nA new invoice has been created for your subscription.\n\nPlan: ${plan?.name || 'Subscription'}\nAmount: ${symbol}${invoice.total.toFixed(2)}\nDue Date: ${new Date(invoice.dueDate).toLocaleDateString()}\n\nPlease complete payment before the due date to avoid service interruption.`
  });
}

export async function sendPaymentReceivedEmail(toEmail, username, payment) {
  const config = loadConfig();
  const panelName = config.panel?.name || 'Sodium Panel';
  const panelUrl = config.panel?.url || '';
  const symbol = config.billing?.currencySymbol || '$';
  ...
  const content = `
    ...
    <div style="background-color: #0a0a0a; border: 1px solid #27272a; border-radius: 8px; padding: 16px; margin-bottom: 20px;">
      <table width="100%" style="color: #a1a1aa; font-size: 14px;">
        <tr>
          <td style="padding: 4px 0;">Amount:</td>
          <td style="text-align: right; color: #22c55e; font-weight: 600;">${symbol}${payment.amount.toFixed(2)}</td>
        </tr>
        <tr>
          <td style="padding: 4px 0;">Method:</td>
          <td style="text-align: right; color: #fafafa;">${payment.method || 'Manual'}</td>
        </tr>
        <tr>
          <td style="padding: 4px 0;">Date:</td>
          <td style="text-align: right; color: #fafafa;">${new Date(payment.createdAt).toLocaleDateString()}</td>
        </tr>
      </table>
    </div>
    ${panelUrl ? emailButton('View Billing', `${panelUrl}/billing`) : ''}
  `;
  
  return sendMail({
    to: toEmail,
    subject: `${panelName} - Payment Received`,
    html: emailTemplate(panelName, 'Payment Received', content, `${panelName} • Thank you for your payment`),
    text: `${panelName} - Payment Received\n\nHi ${username},\n\nThank you for your payment!\n\nAmount: ${symbol}${payment.amount.toFixed(2)}\nMethod: ${payment.method || 'Manual'}\nDate: ${new Date(payment.createdAt).toLocaleDateString()}`
  });
}

export async function sendSubscriptionExpiringEmail(toEmail, username, subscription, daysLeft) {
  const config = loadConfig();
  const panelName = config.panel?.name || 'Sodium Panel';
  const panelUrl = config.panel?.url || '';
  ...
  const content = `
    <p style="margin: 0 0 20px; color: #a1a1aa; font-size: 14px; line-height: 1.6;">
      Hi <strong style="color: #fafafa;">${username}</strong>,<br><br>
      Your subscription is expiring in <strong style="color: #f59e0b;">${daysLeft} day(s)</strong>.
    </p>
    <div style="background-color: #0a0a0a; border: 1px solid #27272a; border-radius: 8px; padding: 16px; margin-bottom: 20px;">
      <p style="margin: 0; color: #a1a1aa; font-size: 14px;">
        Expiration Date: <strong style="color: #fafafa;">${new Date(subscription.currentPeriodEnd).toLocaleDateString()}</strong>
      </p>
    </div>
    ${panelUrl ? emailButton('Renew Subscription', `${panelUrl}/billing`) : ''}
    ${emailNote("Renew your subscription to avoid losing access to your resources.")}
  `;
  
  return sendMail({
    to: toEmail,
    subject: `${panelName} - Subscription Expiring Soon`,
    html: emailTemplate(panelName, 'Subscription Expiring', content, `${panelName} • Billing reminder`),
    text: `${panelName} - Subscription Expiring\n\nHi ${username},\n\nYour subscription is expiring in ${daysLeft} day(s).\n\nExpiration Date: ${new Date(subscription.currentPeriodEnd).toLocaleDateString()}\n\nRenew your subscription to avoid losing access to your resources.`
  });
}

export async function sendTicketReplyEmail(toEmail, username, ticket, isStaffReply) {
  const config = loadConfig();
  const panelName = config.panel?.name || 'Sodium Panel';
  const panelUrl = config.panel?.url || '';
  ...
  const content = `
    <p style="margin: 0 0 20px; color: #a1a1aa; font-size: 14px; line-height: 1.6;">
      Hi <strong style="color: #fafafa;">${username}</strong>,<br><br>
      ${isStaffReply ? 'Support staff has responded to your ticket:' : 'A new message was added to your ticket:'}
    </p>
    <div style="background-color: #0a0a0a; border: 1px solid #27272a; border-radius: 8px; padding: 16px; margin-bottom: 20px;">
      <p style="margin: 0 0 8px; color: #fafafa; font-weight: 600;">
    ...
    ${panelUrl ? emailButton('View Ticket', `${panelUrl}/tickets/${ticket.id}`) : ''}
  `;
  
  return sendMail({
    to: toEmail,
    subject: `${panelName} - Ticket #${ticket.ticketNumber} Updated`,
    html: emailTemplate(panelName, 'Ticket Updated', content, `${panelName} • Support notification`),
    text: `${panelName} - Ticket Updated\n\nHi ${username},\n\n${isStaffReply ? 'Support staff has responded to your ticket:' : 'A new message was added to your ticket:'}\n\n#${ticket.ticketNumber}: ${ticket.subject}\nStatus: ${ticket.status}\nPriority: ${ticket.priority}${panelUrl ? `\n\nView: ${panelUrl}/tickets/${ticket.id}` : ''}`
  });
}
